...
def about(keywords: list[str]):
    ...
    "*** YOUR CODE HERE ***"
    def select(para):
        para_words = split(lower(remove_punctuation(para)))
...
def accuracy(typed: str, source: str) -> float:
    ...
    if(lt == 0 and ls == 0):
        return 100.0
    if(lt == 0 and ls!=0 or ls == 0 and lt!=0):
        return 0.0
    lmin = min(lt, ls)
    for i in range(lmin):
        if(typed_words[i] == source_words[i]):
            correct += 1
    return correct/lt*100.0
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    if typed_word in word_list:
        return typed_word
    diffed = [diff_function(typed_word, word, limit) for word in word_list]
    if min(diffed) > limit:
        return typed_word
    return word_list[diffed.index(min(diffed))]
