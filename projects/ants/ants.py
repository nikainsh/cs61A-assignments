...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        while place:
            if place.bees:
                if k <= self.upper_bound and k >= self.lower_bound:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown:
            ...
            if self.place.bees:
                tmp_bee = random_bee(self.place.bees)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        while tmp_place:
            if tmp_place.ant:
                if tmp_place.ant.is_doubled == False:
                    tmp_place.ant.double()
                    tmp_place.ant.is_doubled = True
                if tmp_place.ant.is_container and tmp_place.ant.ant_contained and tmp_place.ant.ant_contained.is_doubled == False:
                    tmp_place.ant.ant_contained.double()
                    tmp_place.ant.ant_contained.is_doubled = True
